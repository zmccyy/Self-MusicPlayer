...
export function NowPlaying({ open, onOpenChange }: { open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  return createPortal(
    <div
      className={`fixed inset-0 z-40 bg-slate-950/95 backdrop-blur-2xl transition-opacity duration-300 ${
    ...
    >
      ...
      <div className="mx-auto flex h-full max-w-6xl flex-col gap-6 px-6 pb-32 pt-16">
        <div className="flex flex-col items-center gap-4 text-center">
          <div className="flex h-44 w-44 items-center justify-center overflow-hidden rounded-3xl bg-white/10 shadow-2xl">
            {currentSong?.cover ? (
          ...
          <div>
            <div className="text-2xl font-semibold text-white">{currentSong?.name ?? '未选择音乐'}</div>
            <div className="mt-1 text-sm text-slate-400">{currentSong?.artist ?? '—'}</div>
          </div>
          ...
          <div className="flex gap-1 rounded-xl border border-white/10 bg-white/[0.03] p-1">
            {(
              ...
              <button
                ...
                className={
                  tab === value
                    ? 'rounded-lg bg-emerald-500/20 px-4 py-1.5 text-sm text-emerald-300'
                    : 'rounded-lg px-4 py-1.5 text-sm text-slate-400 hover:text-white'
                }
        ...
        {tab === 'lyrics' ? (
          <div ref={lyricsWrapRef} className="min-h-0 flex-1 overflow-y-auto text-center">
            {lines.length === 0 ? (
              <div className="mt-10 text-sm text-slate-500">暂无歌词，可在主面板上传或在线匹配</div>
            ) : (
                ...
                {lines.map((line, idx) => (
                  <button
                    ...
                    className={`block w-full rounded-lg px-4 py-1.5 transition ${
                      idx === activeIndex ? 'text-lg text-white' : 'text-sm text-slate-500 hover:text-slate-300'
                    }`}
                    style={
                      idx === activeIndex
                  ...
                  >
                    ...
                    {line.translation ? (
                      <span className="mt-0.5 block text-xs text-slate-400">{line.translation}</span>
                    ) : null}
                  </button>
                ))}
              </div>
            ...
            <div className="mx-auto max-w-2xl space-y-1 pb-8">
              {playlist.map((song, idx) => (
                <button
                  ...
                  className={`flex w-full items-center gap-3 rounded-xl px-3 py-2 text-left text-sm transition hover:bg-white/[0.06] ${
                    idx === currentIndex ? 'bg-emerald-500/10 text-emerald-200' : 'text-slate-300'
                  }`}
                >
                  <span className="w-6 shrink-0 text-xs text-slate-500">{idx + 1}</span>
                  <span className="min-w-0 flex-1 truncate">
                    {song.name}
                    <span className="ml-2 text-xs text-slate-500">{song.artist}</span>
                  </span>
                  <span className="shrink-0 text-xs text-slate-500">{formatTime(song.duration || 0)}</span>
                </button>
              ))}
            </div>
          </div>
