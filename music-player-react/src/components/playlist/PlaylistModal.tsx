  ...
  return (
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 w-[420px] max-w-[92vw] -translate-x-1/2 -translate-y-1/2 rounded-2xl border border-border-strong bg-elevated p-4 shadow-2xl">
          <Dialog.Title className="text-base font-medium text-text-primary">创建歌单</Dialog.Title>

          <div className="mt-4 flex flex-col gap-3">
            <label className="text-sm">
              <div className="mb-1 text-text-secondary">歌单名称</div>
              <input
            ...
            <label className="text-sm">
              <div className="mb-1 text-text-secondary">分类（可选）</div>
              <input
                className="w-full rounded-xl border border-border-strong bg-surface px-3 py-2 text-sm text-text-primary outline-none ring-emerald-400/50 focus:ring-2"
                value={category}
